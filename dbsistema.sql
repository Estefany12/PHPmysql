...
CREATE TABLE IF NOT EXISTS `dbsistema`.`usuario` (
  ...
  `condicion` VARCHAR(45) NOT NULL DEFAULT '1',
...
CREATE TABLE IF NOT EXISTS `dbsistema`.`ingreso` (
  ...
  `serie_comprobante` VARCHAR(25) NULL,
...
CREATE TABLE IF NOT EXISTS `dbsistema`.`detalle_ingreso` (
  ...
  `precio_vental` DECIMAL(11,2) NOT NULL,
  ...
  CONSTRAINT `fk_detalle_ingreso`
    ...
    REFERENCES `dbsistema`.`detalle_ingreso` (`iddetalle_ingreso`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `dbsistema`.`detalle_venta` (
  ...
  INDEX `fk_detale_venta_articulo_idx` (`idarticulo` ASC),
  CONSTRAINT `fk_detalle_venta`
    FOREIGN KEY (`idventa`)
    REFERENCES `dbsistema`.`venta` (`idventa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_detale_venta_articulo`
    FOREIGN KEY (`idarticulo`)
    REFERENCES `dbsistema`.`articulo` (`idarticulo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
